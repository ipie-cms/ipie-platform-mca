...
The platform is published as `0.1.0-SNAPSHOT` and resolved from `mavenLocal()` first. A snapshot is
mutable, so a service compiles against whatever was last published locally — including uncommitted
platform work. If a service fails on a method that does exist in the source, republish the platform.
This is also why two machines on the same commit can build differently, and why fixed released
versions are recommended before Go-Live.
