...
public final class LayeredArchitectureRules {
    ...
    /**
     * Platform packages that hold an <em>adapter</em> rather than a port. These live outside a
     * service's own base package, so the {@code basePackage + "..x.."} globs above cannot reach
     * them - a service importing one of these is inverting the dependency the wrong way just as
     * surely as importing its own {@code ..persistence..}, and nothing was checking for it.
     *
     * <p>Each entry must have a port a service can depend on instead; where it does, the port
     * lives in a sibling package that is deliberately absent from this list.
     */
    private static final String[] PLATFORM_ADAPTER_PACKAGES = {
            "in.gov.ipie.common.security.keycloak.admin..",
    };
    ...
    /**
     * The broader companion to {@link #domainDoesNotDependOnInfrastructure(String)}: <em>no</em>
     * application-side package may reach into an adapter package, not just {@code ..domain..}.
     *
     * <p>This rule is why the estate had four dependency-inversion leaks and no failing build on
     * 2026-09-05. {@code APPLICATION_PACKAGES} already listed {@code service}, and
     * {@code INFRASTRUCTURE_PACKAGES} already listed {@code persistence}, but the only rule
     * pairing the two sides guarded {@code ..domain..} alone, so
     * {@code ..service.. -> ..persistence..} was never checked and every leak landed there. The
     * domain rule is kept rather than folded in, because a domain class touching infrastructure
     * deserves its own, sharper failure message.
     */
    public static ArchRule applicationDoesNotDependOnInfrastructure(String basePackage) {
        return noClasses().that().resideInAnyPackage(packagesUnder(basePackage, APPLICATION_PACKAGES))
                .should().dependOnClassesThat().resideInAnyPackage(packagesUnder(basePackage, INFRASTRUCTURE_PACKAGES))
                .because("application code must depend on ports, not on adapters - declare an "
                        + "interface in ..repository.. and put the JPA/HTTP/Elasticsearch class "
                        + "behind it (master standards doc, section 16, SOLID/Dependency Inversion)");
    }

    /**
     * The same inversion, one repository away: a platform adapter is still an adapter. Without
     * this, a service satisfies every layering rule while injecting a concrete REST client that
     * ships in {@code ipie-common-libs} - which is what {@code KeycloakUserManagementClient} was
     * until the ports beside it existed.
     */
    public static ArchRule applicationDoesNotDependOnPlatformAdapters(String basePackage) {
        return noClasses().that().resideInAnyPackage(packagesUnder(basePackage, APPLICATION_PACKAGES))
                .should().dependOnClassesThat().resideInAnyPackage(PLATFORM_ADAPTER_PACKAGES)
                .because("depend on the platform's port, not on the class that implements it - "
                        + "the adapter is wired for you by its auto-configuration (master "
                        + "standards doc binding rules, Section 13)");
    }
    ...
    /**
     * Every rule in this class, as one {@link ArchRule} a service applies with a single
     * {@code @ArchTest} field.
     *
     * <p><strong>This exists because copying is not a distribution mechanism.</strong> Each rule
     * used to be wired into each service by hand - eight static fields repeated in four
     * repositories - which made a platform rule opt-in twice over: a new service had to remember
     * the test at all (ipie-iam-service had no {@code ArchitectureTest} for a while despite the
     * standards claiming otherwise), and every existing service had to be edited for a rule to
     * reach it. A service that binds to {@code all()} instead inherits new rules on its next
     * platform version bump, with no edit and nothing to remember.
     *
     * <p>Keep the per-rule methods public: a service that must suppress exactly one rule lists the
     * others individually and says in a comment which one it dropped and why. That is a visible,
     * reviewable exception - the point is that silence is no longer one.
     */
    public static ArchRule all(String basePackage) {
        return CompositeArchRule.of(controllersDoNotAccessRepositoriesDirectly(basePackage))
                .and(domainDoesNotDependOnInfrastructure(basePackage))
                .and(applicationDoesNotDependOnInfrastructure(basePackage))
                .and(applicationDoesNotDependOnPlatformAdapters(basePackage))
                .and(applicationDoesNotDependOnApi(basePackage))
                .and(apiDoesNotExposePersistenceEntities(basePackage))
                .and(exceptionsExtendIpieException(basePackage))
                .and(noCompetingControllerAdvice(basePackage))
                .and(applicationDoesNotCallEventPublisherDirectly(basePackage))
                .and(noHandWrittenPlatformPortAdapters(basePackage))
                // Without this, a failure prints all ten rule descriptions concatenated into one
                // paragraph and the offending class arrives after roughly 3,000 characters of
                // preamble. The short name keeps the report readable; each rule's own "because"
                // reasoning stays here, one method away, where a developer reads it once.
                .as("iPIE layering and binding rules (see LayeredArchitectureRules)");
    }
