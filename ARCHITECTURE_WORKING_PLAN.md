...
| Date | Change |
|---|---|
| 2026-09-05 | **Platform 0.1.0 re-cut over the current tree, because four dependency-inversion leaks had accumulated where no ArchUnit rule was looking, and the fix could not reach the services any other way.** **What was wrong.** `LayeredArchitectureRules` already defined `APPLICATION_PACKAGES` (`service`, `command`, `query`, `domain`) and `INFRASTRUCTURE_PACKAGES` (`persistence`, `search`, `integration`, ...), but the only rule pairing the two guarded `..domain..`, and `APPLICATION_PACKAGES` was used by exactly one rule - `applicationDoesNotDependOnApi` - which checks the *upward* direction. `..service.. -> ..persistence..` was therefore checked by nothing, and four violations lived there with a green build: `RegistrationLookupServiceImpl` injecting four Spring Data repositories and mapping `LookupJpaEntity` itself (its own Javadoc called this a deliberate shortcut, and on its terms it was a fair one - what made it worth undoing is that it put a persistence type in the application layer); `PillarLinkServiceImpl` holding two concrete HTTP clients plus `PillarLinkingProperties`, so it assembled a provider's authorization URL itself; and `RoleServiceImpl` / `AccountProvisioningServiceImpl` injecting the concrete `KeycloakUserManagementClient`. **The last two could never have been caught**, because every rule is scoped `basePackage + "..x.."` and that class lives in `in.gov.ipie.common.security.keycloak.admin` - outside the search space, not missed inside it. **What was added.** `applicationDoesNotDependOnInfrastructure` (the domain rule widened to all application packages, kept alongside it so a domain violation still gets its sharper message) and `applicationDoesNotDependOnPlatformAdapters` (a named `PLATFORM_ADAPTER_PACKAGES` list, so a platform adapter with a port beside it can be forbidden by name). Verified by planting a `service -> persistence` probe class, watching the build fail on it, and removing it - not by assuming a new rule works. **New ports:** `AccountDirectory` and `RealmRoleDirectory` over the one Keycloak adapter, split because the two halves need different Keycloak privileges (`manage-users` vs `manage-realm`) and are granted to different service clients; `LookupRepository` keyed by a `LookupCatalogue` enum, so a new catalogue is one enum constant and one map entry rather than a method on the port and every implementation; `PillarIdpTokenClient` and `PillarIdpWriteBack`, with the HTTP classes renamed to the `Http`-prefixed adapter convention `ElasticsearchUserSearchIndex` already set. **The distribution fix, which matters more than any single rule.** The rule set used to reach services by being retyped into each one - eight `@ArchTest` fields across four repositories - making a platform rule opt-in twice: a new rule reached a service only if someone edited its copy, and a new service only if someone created the file at all, which is how ipie-iam-service ran with no `ArchitectureTest` while MASTER_CODE_STANDARDS claimed otherwise. `LayeredArchitectureRules.all(BASE_PACKAGE)` now returns all ten rules as one `CompositeArchRule`, named via `.as(...)` because the unnamed composite printed roughly 3,000 characters of concatenated rule text before naming the offending class. Every service's `ArchitectureTest` is one field; suppressing a rule means listing the others and saying which was dropped. **Two gaps deliberately left open:** a service with no `ArchitectureTest` at all still passes (closing that needs a check in `ipie.java-conventions` for a `@AnalyzeClasses` class, the only layer that sees a service's shape before its tests run), and `new-service.sh` still verifies the `BASE_PACKAGE` rename by grep rather than by running the test - its own header already warns that a missed rename means the rules 'silently assert nothing'. **A third registry with the same shape, found the hard way:** `spotbugs-exclude.xml` allowlists `EI_EXPOSE_REP2` per *exact* type, so renaming a port silently orphaned its entry - and ipie-user-service stayed green anyway because `PillarLinkServiceImpl` uses Lombok's `@RequiredArgsConstructor` and SpotBugs skips `@lombok.Generated` constructors, while ipie-iam-service's hand-written ones failed. Whether a stale entry is caught depends on whether the class happens to use Lombok. That file's own comment warned about this in 2026-08-07; it has now happened twice. **Release mechanics:** `-Pversion=` overrides the root build but not `ipie-build-conventions`, which reads the root `gradle.properties` file directly - and since `ipie-quality-config` ships inside that plugin's jar, the SpotBugs filter change reached nothing until it was republished separately. `deploy/github/publish-platform-0.1.0.sh` became `publish-platform-release.sh <version>` rather than being copied, and refuses a `-SNAPSHOT` argument outright. The earlier local 0.1.0/0.2.0/0.3.0 artifacts were deleted from Maven Local and a single `0.1.0` republished over the current tree - none of them had ever reached a shared registry, so the numbering carried no information anyone outside this machine could rely on. All four services pin `0.1.0`, and the platform tree publishes that same `0.1.0` - there is no snapshot version anywhere in the estate. Publishing the release bytes to GitHub Packages remains blocked on the classic PAT with `write:packages` that has blocked it since 2026-09-02, so 0.1.0 is a Maven Local release on this machine only. |
