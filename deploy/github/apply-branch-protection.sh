...
#
# Every repository here holds a single "Initial commit" with all five long-lived branches created
# from it, so the whole estate presents one starting point. The branching model requires the same
# of all six.
REPOS=(
  ipie-platform-mca
  ipie-iam-service
